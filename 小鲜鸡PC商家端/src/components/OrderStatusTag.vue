...
const tagType = computed(() => {
  ...
  const map = { pending: 'info', paid: '', accepted: 'warning', weighed: 'warning', card_assigned: '', processing: '', ready: 'success', delivering: '', completed: 'success', cancelled: 'danger', refundFailed: 'danger' }
  return map[props.status] || 'info'
})
...
const text = computed(() => {
  ...
  const map = {
    ...
    card_assigned: '已挂牌', processing: '处理中', ready: '待取货', delivering: '配送中',
    completed: '已完成', cancelled: '已取消', refundFailed: '退款异常'
  }
  return map[props.status] || props.status
})
</script>
